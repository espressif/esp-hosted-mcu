...
#include "rpc_wrap.h"
// Was: #include "esp_private/wifi.h" → see h_priv_event.h
...
{
	H_LOGE(TAG, TAG, "Init event not received within timeout, Resetting myself");
...
static void transport_driver_event_handler(uint8_t event)
{
	switch(event)
	{
		case TRANSPORT_TX_ACTIVE:
		{
			...
			H_LOGI(TAG, TAG, "Base transport is set-up, TRANSPORT_TX_ACTIVE");
...
{
	H_LOGI(TAG, TAG, "set_transport_state: %u", state);
...
static void transport_drv_init(void)
{
	...
	H_LOGD(TAG, TAG, "Bus handle: %p", bus_handle);
	assert(bus_handle);
#if H_NETWORK_SPLIT_ENABLED
	H_LOGI(TAG, TAG, "Network split enabled. Port ranges- Host:TCP(%d-%d), UDP(%d-%d), Slave:TCP(%d-%d), UDP(%d-%d)",
...
h_err_t teardown_transport(void)
{
	...
	H_LOGI(TAG, TAG, "TRANSPORT_INACTIVE");
...
h_err_t transport_drv_reconfigure(void)
{
	...
	H_LOGI(TAG, TAG, "Attempt connection with slave: retry[%u]", retry_slave_connection);
	...
	if (!init_timeout_timer) {
		...
		if (!init_timeout_timer) {
			H_LOGE(TAG, TAG, "Failed to create init timeout timer");
			return H_FAIL;
		}
		H_LOGI(TAG, TAG, "Started host communication init timer of %u millisec", H_HOST_RESTART_NO_COMMUNICATION_WITH_SLAVE_TIMEOUT_MS);
	}
#endif

	int retry_power_save_recover = 5;
	if (esp_hosted_woke_from_power_save()) {
		H_LOGI(TAG, TAG, "Waiting for power save to be off");
	...
	if (!is_transport_tx_ready()) {
		if (H_OK != ensure_slave_bus_ready(bus_handle)) {
			H_LOGE(TAG, TAG, "ensure_slave_bus_ready failed");
			return H_FAIL;
		}
		transport_state = TRANSPORT_RX_ACTIVE;
		H_LOGI(TAG, TAG, "Waiting for esp_hosted slave to be ready");
		while (!is_transport_tx_ready()) {
			if (retry_slave_connection < MAX_RETRY_TRANSPORT_ACTIVE) {
				...
				if (retry_slave_connection%50==0) {
					H_LOGI(TAG, TAG, "Not able to connect with ESP-Hosted slave device");
					if (H_OK != ensure_slave_bus_ready(bus_handle)) {
						H_LOGE(TAG, TAG, "ensure_slave_bus_ready failed");
						return H_FAIL;
					}
				...
				H_LOGW(TAG, TAG, "Failed to get ESP_Hosted slave transport up");
				return H_FAIL;
			}
			g_h.funcs->_h_msleep(200);
		}
	} else {
		H_LOGI(TAG, TAG, "Transport is already up");
	}
...
static h_err_t transport_drv_sta_tx(void *h, void *buffer, size_t len)
{
	...
	if (!is_transport_tx_ready() || !chan_arr[ESP_STA_IF]) {
		H_LOGE(TAG, TAG, "Transport TX not ready or STA channel is not available, drop pkt");
	...
	if (!copy_buff) {
		H_LOGW(TAG, TAG, "STA TX: mempool_alloc failed, dropping pkt (len=%u)", len);
#if defined(ESP_ERR_ESP_NETIF_TX_FAILED)
		return ESP_ERR_ESP_NETIF_TX_FAILED;
#else
		return ESP_ERR_ESP_NETIF_NO_MEM;
...
static h_err_t transport_drv_ap_tx(void *h, void *buffer, size_t len)
{
	...
	if (!is_transport_tx_ready() || !chan_arr[ESP_AP_IF]) {
		H_LOGE(TAG, TAG, "Transport TX not ready or AP channel is not available, drop pkt");
	...
	if (!copy_buff) {
		H_LOGW(TAG, TAG, "AP TX: mempool_alloc failed, dropping pkt (len=%u)", len);
#if defined(ESP_ERR_ESP_NETIF_TX_FAILED)
		return ESP_ERR_ESP_NETIF_TX_FAILED;
#else
		return ESP_ERR_ESP_NETIF_NO_MEM;
...
h_err_t transport_drv_serial_tx(void *h, void *buffer, size_t len)
{
	...
	if (!is_transport_tx_ready() || !chan_arr[ESP_SERIAL_IF]) {
		...
		H_LOGE(TAG, TAG, "Transport TX not ready or serial channel is not available, drop pkt");
...
{
	H_LOGD(TAG, TAG, "Adding channel IF[%u]: S[%u] Tx[%p] Rx[%p]", if_type, secure, tx, rx);
	transport_channel_t *channel = NULL;

	ESP_ERROR_CHECK(if_type >= ESP_MAX_IF);

	if (!tx || !rx) {
		H_LOGE(TAG, TAG, "%s fail for IF[%u]: tx or rx is NULL", __func__, if_type );
		return NULL;
	}

	if (chan_arr[if_type]) {
		H_LOGW(TAG, TAG, "Channel [%u] already created, replacing with new callbacks", if_type);
	...
	default:
		//*tx = transport_drv_tx;
		H_LOGW(TAG, TAG, "Not yet supported ESP_Hosted interface for if_type[%u]", if_type);
		return NULL;
	}
	...
	H_LOGD(TAG, TAG, "Add ESP-Hosted channel IF[%u]: S[%u] Tx[%p] Rx[%p]",
...
{
	H_LOGI(TAG, TAG, "capabilities: 0x%x",cap);
}
...
static uint32_t process_ext_capabilities(uint8_t * ptr)
{
	...
	cap = (uint32_t)ptr[0] +
		...
		((uint32_t)ptr[2] << 16) +
		((uint32_t)ptr[3] << 24);
	H_LOGI(TAG, TAG, "extended capabilities: 0x%"PRIx32,cap);
...
{
	H_LOGI(TAG, TAG, "Features supported are:");
	if (cap & ESP_WLAN_SDIO_SUPPORT)
		H_LOGI(TAG, TAG, "\t * WLAN");
	if (cap & ESP_BT_UART_SUPPORT)
		H_LOGI(TAG, TAG, "\t   - HCI over UART");
	if (cap & ESP_BT_SDIO_SUPPORT)
		H_LOGI(TAG, TAG, "\t   - HCI over SDIO");
	if (cap & ESP_BT_SPI_SUPPORT)
		H_LOGI(TAG, TAG, "\t   - HCI over SPI");
	if ((cap & ESP_BLE_ONLY_SUPPORT) && (cap & ESP_BR_EDR_ONLY_SUPPORT))
		H_LOGI(TAG, TAG, "\t   - BT/BLE dual mode");
	else if (cap & ESP_BLE_ONLY_SUPPORT)
		H_LOGI(TAG, TAG, "\t   - BLE only");
	else if (cap & ESP_BR_EDR_ONLY_SUPPORT)
		H_LOGI(TAG, TAG, "\t   - BR EDR only");
}
...
static void print_ext_capabilities(uint8_t * ptr)
{
	...
	H_LOGI(TAG, TAG, "Extended Features supported:");
#if H_SPI_HD_HOST_INTERFACE
	if (cap & ESP_SPI_HD_INTERFACE_SUPPORT_2_DATA_LINES)
		H_LOGI(TAG, TAG, "\t * SPI HD 2 data lines interface");
	if (cap & ESP_SPI_HD_INTERFACE_SUPPORT_4_DATA_LINES)
		H_LOGI(TAG, TAG, "\t * SPI HD 4 data lines interface");
	if (cap & ESP_WLAN_SUPPORT)
		H_LOGI(TAG, TAG, "\t * WLAN");
	if (cap & ESP_BT_INTERFACE_SUPPORT)
		H_LOGI(TAG, TAG, "\t * BT/BLE");
#elif H_UART_HOST_TRANSPORT
	if (cap & ESP_WLAN_UART_SUPPORT)
		H_LOGI(TAG, TAG, "\t * WLAN over UART");
	if (cap & ESP_BT_VHCI_UART_SUPPORT)
		H_LOGI(TAG, TAG, "\t * BT over UART (VHCI)");
#else
	H_LOGI(TAG, TAG, "\t No extended features. capabilities[%" PRIu32 "]", cap);
#endif
}
...
static void process_event(uint8_t *evt_buf, uint16_t len)
{
	...
	if (event->event_type == ESP_PRIV_EVENT_INIT) {
		...
		H_LOGW(TAG, TAG, "Drop unknown event\n\r");
	}
}
...
static h_err_t get_chip_str_from_id(int chip_id, char* chip_str)
{
	...
	case ESP_PRIV_FIRMWARE_CHIP_ESP32H4:
		...
		break;
	default:
		H_LOGW(TAG, TAG, "Unsupported chip id: %u", chip_id);
...
#else
	H_LOGW(TAG, TAG, "Incorrect host config for ESP slave chipset[%x]", chip_type);
#endif
	char slave_str[20] = {0};
...
static int compare_fw_version(uint32_t slave_version)
{
	...
	if (host_version == slave_version) {
		...
		H_LOGW(TAG, TAG, "Version mismatch: Host [%u.%u.%u] > Co-proc [%u.%u.%u] ==> Upgrade co-proc to avoid RPC timeouts",
	    ...
	    // host version < slave version
#ifndef CONFIG_ESP_HOSTED_FW_VERSION_MISMATCH_WARNING_SUPPRESS
		H_LOGW(TAG, TAG, "Version mismatch: Host [%u.%u.%u] < Co-proc [%u.%u.%u] ==> Upgrade host to avoid compatibility issues",
			ESP_HOSTED_VERSION_PRINTF_ARGS(host_version), ESP_HOSTED_VERSION_PRINTF_ARGS(slave_version));
#endif
		return 1;
	}
...
#define LENGTH_1_BYTE 1
	...
	/* TLV - Board type */
	H_LOGI(TAG, TAG, "Slave chip Id[%x]", ESP_PRIV_FIRMWARE_CHIP_ID);
	...
	*pos = low_thr_thesh;                              pos++;len++;
	...
	H_LOGI(TAG, TAG, "raw_tp_dir[%s], flow_ctrl: low[%u] high[%u]",
...
{
	H_LOGI(TAG, TAG, "transport_delayed_init");
	rpc_start();
	/* Add up cli */
#ifdef H_ESP_HOSTED_CLI_ENABLED
...
static int process_init_event(uint8_t *evt_buf, uint16_t len)
{
	...
	if (init_timeout_timer) {
		g_h.funcs->_h_timer_stop(init_timeout_timer);
		init_timeout_timer = NULL;
		H_LOGI(TAG, TAG, "Init event received within timeout, cleared timer");
	}
#endif

	pos = evt_buf;
	H_LOGD(TAG, TAG, "Init event length: %u", len);
	if (len > 64) {
		H_LOGE(TAG, TAG, "Init event length: %u", len);
#if H_TRANSPORT_IN_USE == H_TRANSPORT_SPI
		H_LOGE(TAG, TAG, "Seems incompatible SPI mode try changing SPI mode. Asserting for now.");
#endif
		assert(len < 64);
	}
		...
		if (*pos == ESP_PRIV_CAPABILITY) {
...
#else
			if (*(pos + 2))
				H_LOGW(TAG, TAG, "Slave enabled Raw Throughput Testing, but not enabled on Host");
#endif
		} else if (*pos == ESP_PRIV_RX_Q_SIZE) {
			H_LOGD(TAG, TAG, "slave rx queue size: %u", *(pos + 2));
		} else if (*pos == ESP_PRIV_TX_Q_SIZE) {
			H_LOGD(TAG, TAG, "slave tx queue size: %u", *(pos + 2));
			...
			slave_fw_version =
				*(pos + 2) |
				...
				(*(pos + 4) << 16) |
				(*(pos + 5) << 24);
			H_LOGD(TAG, TAG, "slave fw version: 0x%08" PRIx32, slave_fw_version);
...
#endif
			H_LOGI(TAG, TAG, "SDIO mode: slave: %s, host: %s",
					slave_sdio_mode ? "streaming" : "packet",
					host_sdio_mode ? "streaming" : "packet");

			if (slave_sdio_mode && !host_sdio_mode) {
				H_LOGE(TAG, TAG, "SDIO mode mismatch: slave is in streaming mode, but host is in packet mode. Aborting.");
				assert(0);
			}
#endif
		} else {
			H_LOGD(TAG, TAG, "Unsupported EVENT: %2x", *pos);
		}
		pos += (tag_len+2);
		len_left -= (tag_len+2);
	}
	...
	if ((chip_type != ESP_PRIV_FIRMWARE_CHIP_ESP32) &&
		...
		H_LOGI(TAG, TAG, "ESP board type is not mentioned, ignoring [%d]\n\r", chip_type);
		chip_type = ESP_PRIV_FIRMWARE_CHIP_UNRECOGNIZED;
		return -1;
	} else {
		H_LOGI(TAG, TAG, "ESP board type is : %d \n\r", chip_type);
	}
	...
	if (ext_cap) {
		...
		if (H_SPI_HD_HOST_NUM_DATA_LINES == 4) {
			...
			if (ext_cap & ESP_SPI_HD_INTERFACE_SUPPORT_4_DATA_LINES) {
				...
				H_LOGI(TAG, TAG, "configure SPI_HD interface to use 4 data lines");
				g_h.funcs->_h_spi_hd_set_data_lines(H_SPI_HD_CONFIG_4_DATA_LINES);
			} else {
				// slave configured to use 2 bits
				H_LOGI(TAG, TAG, "configure SPI_HD interface to use 2 data lines");
			...
			if (ext_cap & ESP_SPI_HD_INTERFACE_SUPPORT_4_DATA_LINES) {
				// slave configured to use 4 bits
				H_LOGI(TAG, TAG, "SPI_HD on slave uses 4 data lines but Host is configure to use 2 data lines");
				g_h.funcs->_h_spi_hd_set_data_lines(H_SPI_HD_CONFIG_2_DATA_LINES);
			} else {
				// slave configured to use 2 bits
				H_LOGI(TAG, TAG, "configure SPI_HD interface to use 2 data lines");
