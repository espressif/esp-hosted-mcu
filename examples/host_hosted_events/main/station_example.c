...
/**
 * these events will inform netif to close its current network connection
 * so it is ready for the next sta connection
 */
void example_wifi_sta_netif_close(void)
{

	// broadcast a wi-fi disconnect event so NETIF is aware that the connection went down
	do_not_reconnect = true; // we don't try to reconnect
	...
	esp_event_post(WIFI_EVENT,
			...
			portMAX_DELAY);
	// sleep for a while to give NETIF time to react to the event
	vTaskDelay(100 / portTICK_PERIOD_MS);

	// this informs the event handler not to try to reconnect
	do_not_reconnect = false;

	// broadcast a wi-fi stop event so NETIF is aware the station has stopped
	esp_event_post(WIFI_EVENT,
			WIFI_EVENT_STA_STOP,
			NULL,
			0,
