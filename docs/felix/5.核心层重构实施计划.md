...
**Files:**
- Create: `host/core/src/h_rpc_wrap.c` (from `host/api/src/esp_wifi_weak.c`)
...
- [ ] **Step 2: 搬迁 RPC wrapper（从 `esp_wifi_weak.c`）**

```bash
cp host/api/src/esp_wifi_weak.c host/core/src/h_rpc_wrap.c
```

替换：
- `esp_err_t` → `h_err_t`
- `ESP_OK` / `ESP_FAIL` → `H_OK` / `H_FAIL`
- `esp_wifi_*` 函数名 → `h_wifi_remote_*`（与 RPC 核心对接）
...
```bash
git add host/core/src/h_rpc_wrap.c host/core/src/h_api.c \
        host/core/include/h_public/h_wifi_api.h
git commit -m "feat(core): add public Wi-Fi API and RPC wrapper

Replace esp_wifi_weak.c with h_rpc_wrap.c using h_err_t and h_wifi_* types."
```
...
host/core/src/
  ...
  h_rpc_wrap.c       ← Wi-Fi RPC 包装器（from esp_wifi_weak.c）
