...
| `h_osal_contract_t` + `h_transport_contract_t` | **重构设计** | ④ vtable 合约 + ⑤ OSAL 适配 | 拆分自 `hosted_osi_funcs_t`，见 `docs/felix/4.ESP-Hosted Host 通用移植架构设计.md` |
| `h_wrapper.h` | **重构设计** | ③ Wrapper 二次封装 | 统一命名空间的宏封装层 |
| `sdkconfig.ci.*` / `Kconfig` | **现有代码** | ① 条件编译 + ⑦ 代码生成 | Kconfig 选传输模式 |
| `host/port/` 目录 | **部分现有** | ③ vtable 合约 + 目录分离 | 目前只有 `port/esp/freertos/`，重构后扩展为多平台 |

> **注意**：标为"重构设计"的模块尚未在代码中实现，目前仅存在于架构设计文档（`docs/felix/4.*`）中。
