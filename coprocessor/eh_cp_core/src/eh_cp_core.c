...
static volatile uint8_t datapath_open_pending = 0;
volatile uint8_t station_connected = 0;
volatile uint8_t softap_started = 0;


/* Default V1 until host ACK upgrades to V2; readers safe before negotiation. */
...
static esp_err_t host_to_slave_reconfig(uint8_t *evt_buf, uint16_t len)
{
	...
	while (len_left) {
		...
		if (*pos == HOST_CAPABILITIES) {
			...
			case ESP_TEST_RAW_TP__ESP_TO_HOST:
				ESP_LOGI(TAG, "RawTP host request: ESP->Host");
				break;
			case ESP_TEST_RAW_TP__HOST_TO_ESP:
				ESP_LOGI(TAG, "RawTP host request: Host->ESP");
				break;
			case ESP_TEST_RAW_TP__BIDIRECTIONAL:
				ESP_LOGI(TAG, "RawTP host request: Bidirectional");
				break;
			case ESP_TEST_RAW_TP_NONE:
				ESP_LOGI(TAG, "RawTP host request: Stop");
				break;
			default:
				ESP_LOGW(TAG, "RawTP host request: unsupported config 0x%02x", *(pos + 2));
			}
			...
			if (acked_ver == ESP_HOSTED_HDR_VERSION_V2) {
				...
				/* Re-init frame component for V2 or encode/decode stays on V1. */
...
#elif CONFIG_EH_TRANSPORT_CP_SPI
				frame_cfg = (eh_frame_cfg_t)EH_FRAME_CFG_CP_LINUX_802_3_SPI_DEFAULT;
#elif CONFIG_EH_TRANSPORT_CP_SPI_HD
				frame_cfg = (eh_frame_cfg_t)EH_FRAME_CFG_CP_LINUX_802_3_SPI_HD_DEFAULT;
#elif CONFIG_EH_TRANSPORT_CP_UART
				frame_cfg = (eh_frame_cfg_t)EH_FRAME_CFG_CP_LINUX_802_3_UART_DEFAULT;
#endif
#endif
			...
			if (acked_rpc_ver == ESP_HOSTED_RPC_VERSION_V3) {
				rpc_ver_negotiated = ESP_HOSTED_RPC_VERSION_V3;
				ESP_LOGI(TAG, "RPC V3 negotiated (unified proto, msg_id 0x400+)");
			} else {
		...
		} else if (*pos == EH_PRIV_RPC_VERSION) {
			/* Phase-D strict-match RPC version negotiation. Peer host
			 * advertises its wire version; mismatch is fatal. */
	...
	ESP_LOGI(TAG, "Negotiation complete: wire_hdr=V%u (%s), rpc=V%u (%s)",
		...
		rpc_ver_negotiated,
		(rpc_ver_negotiated == ESP_HOSTED_RPC_VERSION_V3) ? "unified 0x400+ (V3)" : "variant-specific");
...
#if TEST_RAW_TP
	switch (payload[0]) {
	case ESP_PRIV_CMD_RAW_TP_ESP_TO_HOST:
		ESP_LOGI(TAG, "Legacy fg raw-TP command: ESP->Host");
		eh_cp_process_transport_test_caps(ESP_TEST_RAW_TP__ESP_TO_HOST);
		break;
	case ESP_PRIV_CMD_RAW_TP_HOST_TO_ESP:
		ESP_LOGI(TAG, "Legacy fg raw-TP command: Host->ESP");
		eh_cp_process_transport_test_caps(ESP_TEST_RAW_TP__HOST_TO_ESP);
		break;
	default:
		ESP_LOGW(TAG, "Unsupported legacy priv command 0x%02x", payload[0]);
		break;
	}
#else
	ESP_LOGW(TAG, "Legacy priv command 0x%02x ignored (TEST_RAW_TP disabled)", payload[0]);
#endif
}
...
static void process_rx_pkt(interface_buffer_handle_t *buf_handle)
{
	...
	if (buf_handle->if_type == ESP_PRIV_IF) {
		if (buf_handle->pkt_type == ESP_PACKET_TYPE_COMMAND)
		...
		if (buf_handle->seq_num != s_serial_rx.cur_seq_no) {
			...
			if (s_serial_rx.len > 0 && s_serial_rx.data) {
				eh_cp_dispatch_rx(ESP_SERIAL_IF,
				                                s_serial_rx.data,
		...
		if (!(buf_handle->flags & MORE_FRAGMENT)) {
			...
			/* Consumer takes its own copy; safe to free reassembly buffer. */
			free(s_serial_rx.data);
			s_serial_rx.data = NULL;
			s_serial_rx.len = 0;
			s_serial_rx.cur_seq_no = 0;
		...
		eh_cp_dispatch_rx(ESP_HCI_IF, payload, payload_len, NULL);
	}
#if TEST_RAW_TP
	...
	switch(val) {
		case ESP_OPEN_DATA_PATH:
			...
			if (if_handle) {
				...
				datapath_open_pending = 1;
				ESP_EARLY_LOGI(TAG, "OPEN_DATA_PATH before if_handle ready; deferring");
			}
			break;
...
static void host_reset_task(void* pvParameters)
{
	...
	while (g_cp_running) {
		...
		/* Wait until all features finish init before advertising caps. */
		assert(g_auto_feat_init_done_eg);
...
esp_err_t eh_cp_init_internal(void)
{
...
#if EH_CP_FEAT_WIFI_READY
    EH_CHECK_OK_WARN(eh_cp_rx_register(ESP_STA_IF, default_wlan_sta_rx_callback));
    EH_CHECK_OK_WARN(eh_cp_rx_register(ESP_AP_IF, wlan_ap_rx_callback));
#endif
    ...
    assert(xTaskCreate(auto_feat_init_task, "auto_feat_init_task",
            ...
            EH_CP_TASK_PRIO_DEFAULT, NULL) == pdTRUE);
#else
    /* Skip wait so host_reset_task doesn't block when auto-init is off. */
...
esp_err_t eh_cp_init(void)
{
    ...
    /* Default event loop must exist before features register handlers. */
...
#endif
#if EH_CP_BT_UART
	ESP_LOGI(TAG, "                Transport used :: %s + BT-UART", tp);
#else
	ESP_LOGI(TAG, "                Transport used :: %s only", tp);
#endif
	ESP_LOGI(TAG, "*********************************************************************");
}

/* Core contributes transport-level bits; features add their own in init(). */
static void populate_core_caps(void)
{
    uint8_t  caps     = 0;
    uint32_t ext_caps = 0;
...
#if EH_CP_FEAT_BT_READY
    /* BT bits accumulated by eh_cp_bt_init(). */
#endif
  ...
  #elif (CONFIG_EH_TRANSPORT_CP_SPI_HD_NUM_DATA_LINES == 1)
    ESP_LOGI(TAG, "- SPI HD 1-bit interface");
    ext_caps |= EH_TRANSPORT_CP_SPI_HD_1_DATA_LINE;
  #else
