...
static const char* TAG = "ehcp_wifi_rpc";
...
#endif
/* Slave always decodes reserved field — host may or may not have it enabled. */
#define EH_CP_DECODE_WIFI_RESERVED_FIELD 1
...
/* Compare only fields set by req_wifi_init() — decides if reinit is needed. */
...
/* Merge host-supplied wifi_init_config with CP defaults; logs changed fields. */
...
esp_err_t req_wifi_start(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	/* Replay STA/AP start events for hosts waking from deep sleep. */
	wifi_mode_t mode;
	int event_id;
...
esp_err_t req_wifi_connect(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	if (eh_cp_feat_wifi_is_station_connected()) {
		ESP_LOGI(TAG, "connect recvd while already connected; replaying last real connected event");
...
esp_err_t req_wifi_set_config(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	if (req_payload->iface == WIFI_IF_STA) {
		...
		p_a_sta->rm_enabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_1_rm_enabled, p_c_sta->bitmask);
		p_a_sta->btm_enabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_1_btm_enabled, p_c_sta->bitmask);
		p_a_sta->mbo_enabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_1_mbo_enabled, p_c_sta->bitmask);
		...
		p_a_sta->he_mcs9_enabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_he_mcs9_enabled_BIT, p_c_sta->he_bitmask);
		p_a_sta->he_su_beamformee_disabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_he_su_beamformee_disabled_BIT, p_c_sta->he_bitmask);
		p_a_sta->he_trig_su_bmforming_feedback_disabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_he_trig_su_bmforming_feedback_disabled_BIT, p_c_sta->he_bitmask);
		p_a_sta->he_trig_mu_bmforming_partial_feedback_disabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_he_trig_mu_bmforming_partial_feedback_disabled_BIT, p_c_sta->he_bitmask);
		p_a_sta->he_trig_cqi_feedback_disabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_he_trig_cqi_feedback_disabled_BIT, p_c_sta->he_bitmask);

#if EH_CP_IDF_GE_5_5
		p_a_sta->vht_su_beamformee_disabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_vht_su_beamformee_disabled, p_c_sta->he_bitmask);
		p_a_sta->vht_mu_beamformee_disabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_vht_mu_beamformee_disabled, p_c_sta->he_bitmask);
		p_a_sta->vht_mcs8_enabled = EH_CP_GET_BIT(WIFI_STA_CONFIG_2_vht_mcs8_enabled, p_c_sta->he_bitmask);
#endif
...
esp_err_t req_wifi_get_config(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	case WIFI_IF_STA: {
		...
		if (p_a_sta->rm_enabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_1_rm_enabled, p_c_sta->bitmask);

		if (p_a_sta->btm_enabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_1_btm_enabled, p_c_sta->bitmask);

		if (p_a_sta->mbo_enabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_1_mbo_enabled, p_c_sta->bitmask);

		if (p_a_sta->ft_enabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_1_ft_enabled, p_c_sta->bitmask);

		if (p_a_sta->owe_enabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_1_owe_enabled, p_c_sta->bitmask);
		...
		if (p_a_sta->he_dcm_set)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_he_dcm_set_BIT, p_c_sta->he_bitmask);
		...
		if (p_a_sta->he_mcs9_enabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_he_mcs9_enabled_BIT, p_c_sta->he_bitmask);

		if (p_a_sta->he_su_beamformee_disabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_he_su_beamformee_disabled_BIT, p_c_sta->he_bitmask);

		if (p_a_sta->he_trig_su_bmforming_feedback_disabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_he_trig_su_bmforming_feedback_disabled_BIT, p_c_sta->he_bitmask);

		if (p_a_sta->he_trig_mu_bmforming_partial_feedback_disabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_he_trig_mu_bmforming_partial_feedback_disabled_BIT, p_c_sta->he_bitmask);

		if (p_a_sta->he_trig_cqi_feedback_disabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_he_trig_cqi_feedback_disabled_BIT, p_c_sta->he_bitmask);

#if EH_CP_IDF_GE_5_5
		if (p_a_sta->vht_su_beamformee_disabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_vht_su_beamformee_disabled, p_c_sta->he_bitmask);

		if (p_a_sta->vht_mu_beamformee_disabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_vht_mu_beamformee_disabled, p_c_sta->he_bitmask);

		if (p_a_sta->vht_mcs8_enabled)
			EH_CP_SET_BIT(WIFI_STA_CONFIG_2_vht_mcs8_enabled, p_c_sta->he_bitmask);
#endif
...
esp_err_t req_wifi_scan_start(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	if (!req_payload->config || !req_payload->config_set) {
		...
		/* scan_time and its nested active are optional; a host that omits
		 * them must get a clean error, not a NULL deref. */
		RPC_RET_FAIL_IF(!p_c_st || !p_c_st->active);
...
static int copy_ap_record_to_rpc_struct(WifiApRecord *rpc, wifi_ap_record_t *scan)
{
	...
	/*Bitmask*/
	if (scan->phy_11b)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_phy_11b_BIT,rpc->bitmask);

	if (scan->phy_11g)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_phy_11g_BIT,rpc->bitmask);

	if (scan->phy_11n)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_phy_11n_BIT,rpc->bitmask);

	if (scan->phy_lr)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_phy_lr_BIT,rpc->bitmask);

	if (scan->phy_11a)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_phy_11a_BIT,rpc->bitmask);

	if (scan->phy_11ac)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_phy_11ac_BIT,rpc->bitmask);

	if (scan->phy_11ax)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_phy_11ax_BIT,rpc->bitmask);

	if (scan->wps)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_wps_BIT,rpc->bitmask);

	if (scan->ftm_responder)
		EH_CP_SET_BIT(WIFI_SCAN_AP_REC_ftm_responder_BIT,rpc->bitmask);
	...
	if (p_a_he_ap->partial_bss_color)
		EH_CP_SET_BIT(WIFI_HE_AP_INFO_partial_bss_color_BIT,p_c_he_ap->bitmask);

	if (p_a_he_ap->bss_color_disabled)
		EH_CP_SET_BIT(WIFI_HE_AP_INFO_bss_color_disabled_BIT,p_c_he_ap->bitmask);
...
esp_err_t req_wifi_ap_get_sta_list(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	for (int i = 0; i < sta.num; i++) {
		...
		if (sta.sta[i].phy_11b)
			EH_CP_SET_BIT(WIFI_STA_INFO_phy_11b_BIT, p_c_sta_info->bitmask);

		if (sta.sta[i].phy_11g)
			EH_CP_SET_BIT(WIFI_STA_INFO_phy_11g_BIT, p_c_sta_info->bitmask);

		if (sta.sta[i].phy_11n)
			EH_CP_SET_BIT(WIFI_STA_INFO_phy_11n_BIT, p_c_sta_info->bitmask);

		if (sta.sta[i].phy_lr)
			EH_CP_SET_BIT(WIFI_STA_INFO_phy_lr_BIT, p_c_sta_info->bitmask);
...
esp_err_t req_wifi_scan_params(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	if (req_payload->cmd == RPC_CMD__Set) {
		if (!req_payload->is_config_null && req_payload->config) {
			/* scan_time and its nested active are optional nested messages. */
			RPC_RET_FAIL_IF(!req_payload->config->scan_time ||
					!req_payload->config->scan_time->active);
...
esp_err_t req_eap_set_identity(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	RPC_RET_FAIL_IF((size_t)req_payload->len > req_payload->identity.len);
...
esp_err_t req_eap_set_username(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	RPC_RET_FAIL_IF((size_t)req_payload->len > req_payload->username.len);
...
esp_err_t req_eap_set_password(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	RPC_RET_FAIL_IF((size_t)req_payload->len > req_payload->password.len);
...
esp_err_t req_eap_set_new_password(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	RPC_RET_FAIL_IF((size_t)req_payload->len > req_payload->new_password.len);
...
esp_err_t req_eap_set_ca_cert(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	/* The host-supplied *_len must not exceed the bytes actually decoded. */
	RPC_RET_FAIL_IF((size_t)req_payload->ca_cert_len > req_payload->ca_cert.len);
	free_g_ca_cert();
...
esp_err_t req_eap_set_certificate_and_key(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	/* Each host-supplied *_len must not exceed the bytes actually decoded. */
	RPC_RET_FAIL_IF((size_t)req_payload->client_cert_len > req_payload->client_cert.len);
	RPC_RET_FAIL_IF((size_t)req_payload->private_key_len > req_payload->private_key.len);
	RPC_RET_FAIL_IF((size_t)req_payload->private_key_passwd_len > req_payload->private_key_password.len);
...
esp_err_t req_eap_set_pac_file(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	RPC_RET_FAIL_IF((size_t)req_payload->pac_file_len > req_payload->pac_file.len);
...
esp_err_t req_eap_set_fast_params(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	/* eap_fast_config is an optional nested message; a host that omits it
	 * must get a clean error, not a NULL deref. */
	RPC_RET_FAIL_IF(!req_payload->eap_fast_config);
...
esp_err_t req_wifi_sta_twt_config(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	/* config is an optional nested message; reject NULL rather than deref. */
	RPC_RET_FAIL_IF(!req_payload->config);
	wifi_twt_config_t wifi_twt_config = {0};
...
esp_err_t req_wifi_sta_itwt_setup(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	/* setup_config is an optional nested message; reject NULL rather than deref. */
	RPC_RET_FAIL_IF(!p_c_cfg);
	p_a_cfg->setup_cmd = p_c_cfg->setup_cmd;
	p_a_cfg->trigger = EH_CP_GET_BIT(WIFI_ITWT_CONFIG_1_trigger_BIT, p_c_cfg->bitmask_1);
	p_a_cfg->flow_type = EH_CP_GET_BIT(WIFI_ITWT_CONFIG_1_flow_type_BIT, p_c_cfg->bitmask_1);
	...
	p_a_cfg->wake_duration_unit = EH_CP_GET_BIT(WIFI_ITWT_CONFIG_1_wake_duration_unit_BIT, p_c_cfg->bitmask_1);
#if EH_CP_DECODE_WIFI_RESERVED_FIELD
