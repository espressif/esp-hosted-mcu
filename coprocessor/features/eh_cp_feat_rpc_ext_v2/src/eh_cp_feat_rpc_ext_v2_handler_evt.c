...
static const char* TAG = "ehcp_mcu_rpc_evt";
		...
		const wifi_sta_config_t *sta_cfg)
{
	...
	/* RPC_ALLOC/NTFY_ALLOC macros need resp_payload/ntfy_payload in scope. */
	switch (type) {
        ...
        const uint8_t *data, ssize_t len, int event_id)
{
    ...
    if (p_a->config.trigger)
        EH_CP_SET_BIT(WIFI_ITWT_CONFIG_1_trigger_BIT, p_c->config->bitmask_1);

    if (p_a->config.flow_type)
        EH_CP_SET_BIT(WIFI_ITWT_CONFIG_1_flow_type_BIT, p_c->config->bitmask_1);
...
#endif /* EH_CP_FEAT_DEBUG_MEM_MONITOR_READY */
