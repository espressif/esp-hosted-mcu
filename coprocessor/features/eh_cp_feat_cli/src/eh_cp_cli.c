...
static const char *TAG = "ehcp_cli";

static esp_console_repl_t *s_repl = NULL;
static int                 s_cli_started = 0;

/* Priority 80 — runs after core is up. */
...
static int debug_heap_cli_handler(int argc, char *argv[])
{
	printf("\n");
	if (argc < 2) {
		printf("usage: debug-heap <on|off|status>\n");
		return 0;
	}

	if (strcmp(argv[1], "on") == 0) {
		eh_cp_feat_debug_heap_set_enabled(true);
	} else if (strcmp(argv[1], "off") == 0) {
		eh_cp_feat_debug_heap_set_enabled(false);
	} else if (strcmp(argv[1], "status") == 0) {
		printf("debug-heap: %s\n",
		       eh_cp_feat_debug_heap_is_enabled() ? "on" : "off");
	} else {
...
//#if EH_CP_HOST_PS_DEEP_SLEEP_ALLOWED
...
int eh_cp_feat_cli_register_commands(void)
{
	...
	esp_console_cmd_deregister("help");
#if EH_CP_FEAT_HOST_PS_READY
	esp_console_cmd_deregister("wake-up");
#endif

	for (i = 0; i < cmds_num; i++) {
		/* Ensure stale instances from earlier cycles are removed first. */
		esp_console_cmd_deregister(diag_cmds[i].command);
...
esp_err_t eh_cp_feat_cli_start(const char* custom_prompt)
{
	...
	/* esp_console_stop_repl() deinits the console and wipes the command
	 * list, so on every (re)start we must re-register our commands.
	 * Mirrors upstream esp_hosted_cli_start(). */
	...
	ret = esp_console_new_repl_usb_cdc(&hw_config, &repl_config, &s_repl);
	if (ret != ESP_OK) {
	...
	ret = esp_console_new_repl_usb_serial_jtag(&hw_config, &repl_config, &s_repl);
	if (ret != ESP_OK) {
...
void eh_cp_feat_cli_stop(void)
{
	if (!s_cli_started || !s_repl) {
		return;
	}
