...
static const char* TAG = "rpc_linux_802_3_wifi_req";
		...
		CtrlMsg *resp, void *priv_data)
{
	...
	ESP_LOGI(TAG,"ssid %s authmode %d ssid_hidden %d max_conn %d channel %d",
			wifi_config->ap.ssid,
		...
		CtrlMsg *resp, void *priv_data)
{
	...
	if ((eh_cp_feat_wifi_is_softap_started()) &&
		...
		EH_CHECK_OK(esp_wifi_set_mode(WIFI_MODE_APSTA));
		ESP_LOGI(TAG,"softap+station mode set in scan handler");
	} else {
		EH_CHECK_OK(esp_wifi_set_mode(WIFI_MODE_STA));
		...
		CtrlMsg *resp, void *priv_data)
{
	...
	if (!p_vsi->enable) {
		...
		if (!p_vid ||
		    !p_vid->payload.len ||
		    !p_vid->payload.data ||
		    !p_vid->vendor_oui.data ||
		    p_vid->vendor_oui.len < VENDOR_OUI_BUF) {
			ESP_LOGE(TAG, "Invalid parameters");
			return ESP_FAIL;
		}
		...
		CtrlMsg *resp, void *priv_data)
{
	...
	if (!req->req_set_wifi_max_tx_power) {
		ESP_LOGE(TAG, "Invalid parameters");
		resp_payload->resp = FAILURE;
		return ESP_OK;
	}
